// http-transport.ts — Streamable HTTP слой для yandex-calendar-mcp (Node.js).
// Локальная копия mcp_servers/_shared/http-transport.ts: пакет самодостаточен
// для Docker build с context=./yandex-calendar-mcp (esbuild бандлит этот файл).
...
/** Пустой MCP_AUTH_TOKEN — временно без Bearer (только внутренняя сеть). */
...
async function handleHttpRequest(
  ...
  // token === null: auth временно отключён (внутренняя сеть)
